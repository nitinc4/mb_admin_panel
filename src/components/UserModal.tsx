...
interface UserModalProps {
  user: User | null;
  tiers: Tier[]; // Now passing real tiers to populate the dropdown
  onClose: () => void;
  onSave: () => void;
}
...
export default function UserModal({ user, tiers, onClose, onSave }: UserModalProps) {
  ...
  useEffect(() => {
    if (user) {
      setFormData({
        email: user.email,
        full_name: user.name, // maps backend 'name' to the form
        phone: user.phone || '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(url, {
        ...
        body: JSON.stringify(formData), // Sending full_name, is_active etc expected by Express routes
      });
